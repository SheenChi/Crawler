...
class MongoUrlManager:
    ...
    def __init__(self, mongo_ip='localhost', mongo_port=27017,
                    client=None, database_name='Qczj', table_name='question_items'):
        # 连接mongodb
    ...
    def enqueueUrl(self, url, serial_id):
        try:
    ...
    def dequeueUrl(self):
        record = self.table.find_one_and_update(
            ...
            sort=[('serial_id',DESCENDING), ('queue_time', ASCENDING)],
        )
        if record:
            return record
        else:
...
def add_topic_items_2_mongo(index_s, index_e):
    ...
    for serial_id in range(index_s, index_e+1, -1):
        print(serial_id)
